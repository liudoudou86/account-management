<template>
  <div>
    <el-table
      ...
      :data="
        tableDataSTG.filter(data => !sendData || Object.keys(data.tags).some(key => data.tags[key].includes(sendData)))
      "
      border
      style="width: 100%"
      height="330"
    ...
    >
      ...
      <el-table-column prop="tags" label="标签" align="left" width="300">
        <template #default="scope">
          ...
          />
          <el-button v-else size="small" @click="showInput(scope.row)"> + 新标签 </el-button>
        </template>
      </el-table-column>
      <el-table-column fixed="right" label="操作" align="center" width="210">
        <template #default="scope">
          ...
          <el-button
            ...
            >置顶</el-button
          >
          <el-button link type="success" size="small" @click="setCopy(scope.row)">复制</el-button>
          <el-button link type="danger" size="small" @click="delAccount(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  mounted() {
    ...
    this.tableDataSTG = this.rawTableData.filter(item => item.environment === 'STG')
  },
  methods: {
    getTagColor(index) {
    ...
    getTop(row, index) {
      this.tableDataSTG.splice(index, 1)
      this.tableDataSTG.unshift(row)
    },
    handleClose(row, tag) {
