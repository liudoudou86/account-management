...
chrome.runtime.onMessage.addListener(
  ...
  function (request, sender, sendResponse) {
    // 定义全局变量
    let usernameInput = document.querySelector('input[type="text"]') || document.querySelector('input[name="username"]')
    let passwordInput =
      document.querySelector('input[type="password"]') || document.querySelector('input[name="password"]')
    // 此处为了兼容多种类型的按钮
    let submit = document.querySelector('button[type="button"]') || document.querySelector('button[type="submit"]')
